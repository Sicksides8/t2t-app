...
- `t2t_users/{uid}`: perfil, role, subscriptionId, onboardingCompleted, diagnosticCompleted, notificationTokens, bio (opcional). Campos transaccionales de email (Resend): `welcomeEmailSentAt: Timestamp` (idempotencia del welcome email; se setea la primera vez que el endpoint del CRM envía el correo) y `lastStreakWarningEmailSentAt: Timestamp` (último envío del warning anti-churn; usado por el job diario de GitHub Actions para evitar reenviar el mismo día UTC).
